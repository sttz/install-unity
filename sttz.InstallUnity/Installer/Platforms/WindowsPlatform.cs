...
namespace sttz.InstallUnity
{
	...
	public class WindowsPlatform : IInstallerPlatform
	{
		...
		static string ProgramFilesPath { get {
			if (RuntimeInformation.OSArchitecture != System.Runtime.InteropServices.Architecture.X86 && RuntimeInformation.ProcessArchitecture == System.Runtime.InteropServices.Architecture.X86) {
				throw new Exception($"Install-Unity cannot run as X86 on a non-X86 Windows installation");
			}
		...
		{
			return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), UnityInstaller.PRODUCT_NAME);
		}
	    ...
	    public Task<Platform> GetCurrentPlatform()
	    {
	        return Task.FromResult(Platform.Windows);
	    }

	    public Task<UnityReleaseAPIClient.Architecture> GetInstallableArchitectures()
	    {
		    return Task.FromResult(UnityReleaseAPIClient.Architecture.X86_64);
	    }
	    ...
	    Task<(Platform, UnityReleaseAPIClient.Architecture)> IInstallerPlatform.GetCurrentPlatform()
	    {
		    var arch = System.Runtime.InteropServices.RuntimeInformation.OSArchitecture;
		    switch (arch) {
			    case System.Runtime.InteropServices.Architecture.Arm64:
				    return Task.FromResult((Platform.Windows, UnityReleaseAPIClient.Architecture.ARM64));
			    case System.Runtime.InteropServices.Architecture.X64:
				    return Task.FromResult((Platform.Windows, UnityReleaseAPIClient.Architecture.X86_64));
			    default:
				    throw new Exception($"Unexpected Windows architecture: {arch}");
		    }
	    }
...
#pragma warning restore CA1416 // Validate platform compatibility
	    ...
	    {
	        if (item.package is not EditorDownload && !installedEditor)
	        {
